...
| `finetune_vision_layers` | `False` | Vision tower frozen for instruction-style SFT on three grounds: **(a)** the 25k-image dataset is too small to safely re-train a 150M+ parameter vision encoder without eroding general-image perception (catastrophic-forgetting risk); **(b)** vision LoRA approximately doubles trainable parameter count and activation memory; **(c)** Unsloth's vision fine-tuning guide defaults to frozen-vision for instruction-style SFT. Vision-tower provenance differs across the originally-planned model lineup — Gemma 4 (E2B, E4B) uses a 150M custom Vision Transformer pretrained on natural image–text data (not SigLIP, replacing Gemma 3's SigLIP encoder); Qwen 3.5 uses its own general-purpose ViT; MedGemma 1.5 4B uses MedSigLIP-400M, which is medically pretrained on PAD-UFES-20 and SCIN among other dermatology corpora (see §7.16) — but the freeze decision applies uniformly because rationales (a)–(c) are independent of pretraining domain. The MedSigLIP "medical pre-training advantage" framing in earlier drafts of this rationale was incorrect for Gemma 4 and Qwen 3.5, and is irrelevant to the executed campaign per the MedGemma deferral documented in §11.1. |
...
**Per-run batch sizing.** Run 1 (Gemma 4 E2B × label-only) used `per_device_train_batch_size = 4` with `gradient_accumulation_steps = 4` for an effective batch size of 16 — the value standardised in §10.3.1. Runs 2–8 use `per_device_train_batch_size = 8` with `gradient_accumulation_steps = 2`, **preserving the same effective batch size of 16**. Because the optimiser sees the sum of `batch_size × gradient_accumulation_steps` samples per step, this rearrangement is mathematically a wall-clock optimisation only: total optimiser steps, gradient direction (modulo floating-point order-of-operations), and learning-rate schedule are identical. No batch-statistics-dependent operations are involved (the chosen architectures use RMSNorm or LayerNorm, both batch-independent), so the change does not contaminate the architecture × supervision factorial. The motivation is reduced wall-clock per step on the L40S — VRAM headroom on Run 1 (11.1 / 46 GB measured) had been substantially under-utilised at `batch_size = 4`. For Qwen 3.5 9B, this configuration is provisional: if a first-eval-step VRAM measurement exceeds the L40S envelope, the run will fall back to the Run-1 setting (`batch_size = 4`, `gradient_accumulation_steps = 4`) and the deviation will be recorded per-run.

**Format-ablation extension (Run 9).** A ninth run is added to the campaign as a methodological extension that tests the contribution of response-format ordering — the chain-of-thought (reasoning-first) format proposed by Pan et al. (*MedVLM-R1*, arXiv:2502.19634, 2025) versus the JSON-first format used in Runs 2, 4, 6, and 8. The dataset preparation, transformation rule, motivating literature, and pre-registered hypothesis for Run 9 are detailed in §11.7. Run 9 is appended after Run 8 completes; it holds the base model (`unsloth/Qwen3.5-9B`), all hyperparameters, and the random seed constant with Run 8 and varies only the dataset variant (`danielfdias98/derm-reasoning-cot` rather than `derm-reasoning-full-reasoning`). Because Run 9 is strictly additive — it does not substitute for any cell of the 4-architecture × 2-supervision factorial described in §10.5 — it does not contaminate the main supervision-density × architecture ablation; it exists as a parallel single-cell experiment whose result is reported alongside the factorial in the dissertation.
...
**Outcome (recorded 2026-04-29 03:06 UTC, step 1604).** The second evaluation reported `eval_loss = 3.4310`, a decrease of 0.470 (−12.0 %) from `eval_loss = 3.9005` at step 802. This corresponds to **outcome #1** of the three pre-registered alternatives: the model is generalising on the warped absolute scale. The contemporaneous training loss at the evaluation step had also fallen by approximately one half (0.1601 → 0.0830) from its step-802 reading, so the train-to-eval ratio appears to have *widened* (24.4 × → 41.4 ×) despite both quantities improving — a direct consequence of the collator-asymmetry hypothesis: the trained completion tokens respond rapidly to LoRA updates, while the un-trained image and instruction tokens that dominate the absolute `eval_loss` value respond more slowly. The hypothesis is therefore corroborated, and the methodological consequence is unchanged: within-run early-stopping decisions and best-checkpoint selection proceed on **relative** `eval_loss` changes, treating the absolute scale as uninformative. As of step 1604, `pick_best_checkpoint.py` indicates `checkpoint-1600` as the lowest-`eval_loss` artefact to date and would publish that adapter to the Hugging Face Hub if invoked with `--push`.
...
- The first `eval_loss` value (3.9005 at step 802) cannot be interpreted on its absolute scale because of the collator-asymmetry artefact described in §11.4; relative comparison across evaluations remains the basis for both within-run early-stopping decisions and best-checkpoint selection.
- The second evaluation (step 1604) corroborated the §11.4 collator-asymmetry hypothesis by descending to `eval_loss = 3.4310` (−12.0 % relative to the step-802 reading) while the contemporaneous training loss fell by approximately one half — pre-registered outcome #1 was observed, and the lowest-`eval_loss` artefact promoted by `pick_best_checkpoint.py` is currently `checkpoint-1600`.
...
### 11.7 Format-Ablation Extension and Chain-of-Thought Dataset Variant

This subsection documents the preparation of a chain-of-thought (CoT) dataset variant supporting the Run 9 methodological-extension experiment introduced in §11.1.

**Motivation.** The full-reasoning dataset (`danielfdias98/derm-reasoning-full-reasoning`) used in Runs 2, 4, 6, and 8 is structured as a JSON object with the `diagnosis` field appearing first, followed by `category`, `observation`, `morphology`, `colour`, `texture`, `border`, `distribution`, `differentials`, and `reasoning`. This ordering supervises the model to commit to a diagnosis label *before* generating the prose reasoning that justifies it — that is, the supervision encourages post-hoc rationalisation rather than evidence-driven chain-of-thought. Two recent strands of literature suggest this ordering may be sub-optimal for reasoning-heavy tasks:

- Tam et al. (*SLOT: Structuring the Output of Large Language Models*, arXiv:2505.04016, 2025) report a 10–15 % degradation in reasoning-task performance when models are constrained to JSON-mode compared to free-form generation followed by structured conversion. The authors attribute this to format constraints "stealing cognitive resources" from the reasoning task — the model spends compute satisfying schema rather than thinking. Notably, the same paper finds that JSON-mode *helps* classification tasks (e.g., the DDXPlus medical-diagnosis benchmark), so the effect is task-dependent. The dermatological diagnosis task assessed in this dissertation has both classification (Top-1 diagnosis label) and reasoning (clinical chain-of-thought) components, motivating an empirical test rather than a-priori commitment to one format.
- Pan et al. (*MedVLM-R1: Incentivizing Medical Reasoning Capability of Vision-Language Models via Reinforcement Learning*, arXiv:2502.19634, 2025) introduce a reasoning-first format — `<think>{free-form clinical reasoning}</think><answer>{structured answer}</answer>` — and use it as the supervised target for medical visual-language fine-tuning. Their results indicate that reasoning-first formatting, in combination with reinforcement-learning post-training, improves both diagnostic accuracy and reasoning coherence on medical VQA benchmarks. The format has been adopted in subsequent medical-VLM work as a de facto standard for reasoning-aware supervision.

These two findings together motivate a controlled test of whether response-format ordering influences fine-tune quality on the dermatology diagnostic task targeted by this dissertation. The 4 × 2 main factorial (Phase 10.5) holds the format constant across all eight runs, so it cannot answer this question on its own; Run 9 supplies the missing isolated comparison.

**Transformation rule.** The CoT variant applies a *minimal-disruption* transformation to each row of `derm-reasoning-full-reasoning`. The `reasoning` field's prose is moved into a `<think>...</think>` block placed before the structured answer, and all other fields (`diagnosis`, `category`, `observation`, `morphology`, `colour`, `texture`, `border`, `distribution`, `differentials`) are preserved unchanged inside an `<answer>...</answer>` JSON block:

```text
<think>
{original `reasoning` field text — verbatim}
</think>
<answer>
...
The transformation is implemented in `scripts/upload_cot_dataset.py` and is deterministic: no LLM rewriting, no content augmentation, no reordering of fields inside the `<answer>` block, and no addition or removal of training rows. The choice of minimal-disruption rather than full prose-rewriting is methodologically deliberate. Rewriting the reasoning text with a more capable teacher (e.g., GPT-5 reasoning, Claude Opus extended thinking) would introduce a second confound — teacher quality — that would be inseparable from the format-ordering effect when comparing Run 9 to Run 8. By preserving the per-row reasoning content verbatim, the Run 9 vs Run 8 comparison attributes purely to the *position of reasoning relative to the structured answer*. Teacher-quality comparison is reserved as a separate follow-up experiment (Next Steps item 6).

**Hub repository.** The transformed dataset is published as [`danielfdias98/derm-reasoning-cot`](https://huggingface.co/datasets/danielfdias98/derm-reasoning-cot) (CC-BY-NC-SA 4.0, public). Train (25,637 rows) and validation (2,849 rows) splits are inherited unchanged from the source repository, including the per-source attribution (`scin`, `kaggle_dermnet`, `pad_ufes`, `skincap`, `dermnet_nz`) and the random 95/5 split established in Phase 9 (`random_seed = 42`).

**Pre-registered hypothesis for Run 9.** The Run 9 experiment is configured identically to Run 8 except for the dataset (chain-of-thought variant rather than JSON-first). The base model (`unsloth/Qwen3.5-9B`), random seed, learning rate, schedule, LoRA rank and alpha, epochs, effective batch size, and post-hoc best-checkpoint selection (`scripts/pick_best_checkpoint.py`) are held constant with §10.3.2's Qwen 3.5 9B specification. Three pre-registered outcomes (decided before training):

1. Run 9 `eval_loss` is meaningfully lower than Run 8's at every checkpoint, AND post-training Top-1 accuracy on Fitzpatrick17k is higher → reasoning-first formatting is the more effective supervision target at the 9 B scale, and the dissertation will recommend it for future medical-VLM SFT campaigns.
2. Run 9 and Run 8 produce statistically indistinguishable `eval_loss` and downstream-benchmark performance → format ordering is irrelevant when capacity is sufficient, and the dissertation will report this as a useful null result that simplifies the supervision-design decision space for downstream researchers.
3. Run 9 underperforms Run 8 → JSON-first formatting is the more effective target on this dataset, refining the SLOT finding that JSON-mode helps classification-heavy tasks and arguing that the diagnostic-label component of our task dominates over the prose-reasoning component.

The watcher infrastructure (§11.5), the picker (`scripts/pick_best_checkpoint.py`), and the post-training evaluation pipeline (Phase 10.6 metrics on Fitzpatrick17k, Confusion Triads, and MM-Skin VQA) are reused without modification, except for an extension to `run_benchmark.py --lora` that adds an `<answer>...</answer>`-tag-aware extractor for diagnosis labels alongside the existing JSON parser used for Runs 1–8.
